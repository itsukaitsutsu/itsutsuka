/**
 * N2 "exam day" verdict — the single 合否 result that combines all three
...
export const EXAM_DAY_STORAGE_KEY = 'jlpt_n2_exam_day_result';

export const N2_TOTAL_MARKS = 180;
export const N2_PASS_TOTAL = 90;
export const N2_BLOCK_MARKS = 60;
export const N2_BLOCK_BENCHMARK = 19;
...
export const computeExamDayVerdict = (blocks: ExamDayBlocks): ExamDayVerdict => {
  ...
  const benchmarks: Record<BlockKey, boolean> = {
    language: blocks.language >= N2_BLOCK_BENCHMARK,
    reading: blocks.reading >= N2_BLOCK_BENCHMARK,
    listening: blocks.listening >= N2_BLOCK_BENCHMARK,
  };
  const reasons: string[] = [];
  if (total < N2_PASS_TOTAL) {
    reasons.push(
      `総合得点 ${total}/180 is below the 合格点 of ${N2_PASS_TOTAL}/180.`,
    );
  }
  if (!benchmarks.language) {
    reasons.push(
      `言語知識 ${blocks.language}/60 is below the 基準点 of ${N2_BLOCK_BENCHMARK}/60.`,
    );
  }
  if (!benchmarks.reading) {
    reasons.push(
      `読解 ${blocks.reading}/60 is below the 基準点 of ${N2_BLOCK_BENCHMARK}/60.`,
    );
  }
  if (!benchmarks.listening) {
    reasons.push(
      `聴解 ${blocks.listening}/60 is below the 基準点 of ${N2_BLOCK_BENCHMARK}/60.`,
    );
  }
