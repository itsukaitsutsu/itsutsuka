...
import simulationData from '@/lib/realJlptSimulationData.json';
...
const STORAGE_KEY = 'jlpt_n3_simulation_state';

export function RealJlptSimulation() {
  ...
  const scaledScore = Math.round((correctCount / (totalQuestions || 1)) * 180);
  const langKnowledgeScore = (vocabTotal + gramTotal) > 0 ? Math.round(((vocabCorrect + gramCorrect) / (vocabTotal + gramTotal)) * 60) : 60;
  const readingScore = readTotal > 0 ? Math.round((readCorrect / readTotal) * 60) : 60;
  const listeningScore = listenTotal > 0 ? Math.round((listenCorrect / listenTotal) * 60) : 60;

  // Passing criteria: Overall >= 95 AND all sectional components >= 19
  const isPassed =
    scaledScore >= 95 &&
    langKnowledgeScore >= 19 &&
    (readTotal === 0 || readingScore >= 19) &&
    (listenTotal === 0 || listeningScore >= 19);
      ...
      <div className="mx-auto max-w-[960px] px-4 py-8 pb-28 md:px-8 md:py-12 md:pb-16 animate-fadeIn">
        ...
        <div className="rounded-3xl border border-border bg-card p-6 md:p-10 shadow-lg">
          <div className="flex items-start gap-4 border-b border-border pb-6">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                A plain-language summary of the public test-day rules. Always verify against the 受験案内
                given by your test centre — the organisers' instructions always win.
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 space-y-8">
            ...
            <div>
              <h3 className="text-lg font-bold flex items-center gap-2 text-foreground">
                ...
                N3 Timings & Sectional Pass Matrix (per JLPT published guidelines)
              </h3>
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="p-4 rounded-2xl border border-border bg-background text-center">
                  ...
                  <p className="text-2xl font-black text-foreground mt-1">70 Minutes</p>
                ...
                <div className="p-4 rounded-2xl border border-border bg-background text-center">
                  ...
                  <p className="text-2xl font-black text-foreground mt-1">40 Minutes</p>
                  <p className="text-[11px] text-muted-foreground mt-1">One-time broadcast only</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-border flex justify-end">
            ...
            >
              Start Full 3-Section Simulation
                ...
                <div className="mt-5 rounded-2xl border border-primary/30 bg-primary/5 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        Listening Playback (browser voice)
      ...
      <div className="mx-auto max-w-[960px] px-4 py-8 pb-28 md:px-8 md:py-12 md:pb-16 animate-fadeIn">
        <div className={`rounded-3xl border p-6 md:p-8 text-center shadow-lg transition ${
          ...
          <h2 className="text-2xl md:text-3xl font-black text-foreground">
            {isPassed ? 'JLPT N3 PASS (合格) — Simulation Cleared!' : 'JLPT N3 DID NOT PASS (不合格)'}
          </h2>
          <p className="mt-2 text-sm text-muted-foreground max-w-xl mx-auto">
            {isPassed
              ? 'Congratulations! Your score meets the published threshold (≥ 95/180) and clears the 19-point benchmark across all individual sections.'
              : 'Review your breakdown below. To pass the JLPT N3, you must score at least 95/180 points overall AND achieve at least 19/60 in every single section.'}
          </p>
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3 max-w-2xl mx-auto">
            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              <p className="text-[11px] font-mono text-muted-foreground uppercase">Estimated Score</p>
              <p className="text-2xl font-black text-foreground mt-1">{scaledScore} / 180</p>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${scaledScore >= 95 ? 'bg-emerald-500/20 text-emerald-600' : 'bg-destructive/20 text-destructive'}`}>
                Pass req: 95
              </span>
            </div>

            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              <p className="text-[11px] font-mono text-muted-foreground uppercase">Moji・Goi & Bunpou</p>
              <p className="text-2xl font-black text-foreground mt-1">{langKnowledgeScore}/60</p>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${langKnowledgeScore >= 19 ? 'bg-emerald-500/20 text-emerald-600' : 'bg-destructive/20 text-destructive'}`}>
                Benchmark ≥ 19
              </span>
            </div>

            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              <p className="text-[11px] font-mono text-muted-foreground uppercase">Dokkai (Reading)</p>
              <p className="text-2xl font-black text-foreground mt-1">{readTotal > 0 ? `${readingScore}/60` : 'N/A'}</p>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${readingScore >= 19 ? 'bg-emerald-500/20 text-emerald-600' : 'bg-destructive/20 text-destructive'}`}>
                Benchmark ≥ 19
              </span>
            </div>

            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              <p className="text-[11px] font-mono text-muted-foreground uppercase">Chokai (Listening)</p>
              <p className="text-2xl font-black text-foreground mt-1">{listenTotal > 0 ? `${listeningScore}/60` : 'N/A'}</p>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${listeningScore >= 19 ? 'bg-emerald-500/20 text-emerald-600' : 'bg-destructive/20 text-destructive'}`}>
                Benchmark ≥ 19
              </span>
            </div>
          </div>
    ...
    <div className="mx-auto max-w-[1000px] px-5 py-8 pb-28 md:px-10 md:py-12 md:pb-16 animate-fadeIn">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-bold text-muted-foreground flex items-center gap-1">
              N3 Timings & Guidelines
            </span>
          </div>
          <h1 className="mt-2 text-2xl md:text-3xl font-black tracking-tight text-foreground">
            N3-Format Mock Examination
          </h1>
          <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
            Choose between full exam mode or individual timed sections (文字・語彙 30分, 文法・読解 70分, 聴解 40分) with browser-voice listening playback, proper underlines, and passing score evaluation.
          </p>
        </div>
      ...
      <div className="mt-8">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Take the entire 3-part test together, or practice a specific individual section with its exact JLPT timer:
        </p>
          ...
          >
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                All written parts combined under exam-like conditions
              </p>
            </div>
      ...
      {/* Official N3 聴解 recordings/scripts are not redistributed with this app. */}
      <div className="mt-12 rounded-2xl border border-border bg-muted/30 p-5 text-xs leading-6 text-muted-foreground md:p-6">
        <p className="font-bold text-foreground">N3 聴解 practice</p>
        <p className="mt-2">
          聴解 items in these sets are practised with your browser's Japanese voice (synthesised), not with
          official recordings — the organisers never release the audio for past sittings. Official sample
          questions and audio are published by the organisers at{' '}
          <a
