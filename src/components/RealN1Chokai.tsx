...
import rawChokaiData from '@/lib/n1ChokaiData.json';
...
/**
 * N1 聴解 block — original content only.
 *
 * Every script, option and answer in `src/lib/n1ChokaiData.json` was written for
 * this app, in the published N1 聴解 layout (問題1〜5 · 30 items · 55 minutes,
 * the format used from the 2022年第2回 sitting on). Nothing here is copied from,
 * or streamed out of, a real sitting: no official recordings, no official
 * scripts, no official answer keys, no third-party mirrors.
 *
 * Audio is the browser's Japanese voice (speechSynthesis) reading our own
 * transcript, which is also why the "scripts" can be revealed after submitting —
 ...
 *
 * 問題5（統合理解）plays one longer conversation and then asks three questions
 * about it. In the data the follow-up items carry `audio_of: "<first item id>"`
 * and an empty transcript, so Play on any of them replays the shared script and
 * the reveal shows it once.
 */
...
export type ChokaiQuestion = {
  ...
  prompt: string;
  transcript: string;
  /** For 統合理解 follow-ups: the id of the item that owns the shared script. */
  audio_of?: string;
  options: string[];
...
export type ChokaiSitting = {
  ...
  /** Duration of this sitting's 聴解 block, in minutes. */
  examMinutes: number;
  groups: QuestionGroup[];
  /** Legacy fields kept so anything reading them keeps compiling. */
  subCount?: number;
  subChoiceCount?: number;
  subNote?: string;
};
...
export interface RealN1ChokaiProps {
  /**
...
const EXAM_MINUTES = chokaiData.meta.timing_minutes;
const OFFICIAL_SAMPLES_URL = chokaiData.meta.format_reference.url;

/** 問題1–5 layout, with item counts taken straight from the data. */
...
/**
 * 問題5 items share one script: the follow-ups point at the item that owns it,
 * so the voice and the reveal never read the same conversation three times.
 */
const scriptOwner = (items: ChokaiQuestion[], index: number): ChokaiQuestion => {
  const item = items[index];
  if (!item?.audio_of) return item;
  return items.find((candidate) => candidate.id === item.audio_of) ?? item;
};

export function RealN1Chokai({ examDay, replay }: RealN1ChokaiProps = {}) {
  ...
  const playItem = useCallback(
    (index: number) => {
      ...
      if (!item) return;
      const script = scriptOwner(activeSession.items, index);

      window.speechSynthesis.cancel();
      const utterance = new SpeechSynthesisUtterance(speakable(script.transcript));
      ...
      utterance.rate = 1.0; // N1 is the fastest of the five levels
      utterance.onend = () => {
      ...
      <section className="mt-8 border-t border-border pt-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="flex items-center gap-2 text-primary">
              ...
              <span className="text-xs font-black uppercase tracking-wider">N1 聴解 practice block</span>
            </div>
            <h2 className="mt-2 flex items-center gap-2 text-lg font-bold text-foreground">
              3. Sit a {EXAM_MINUTES}-minute Listening Block
            </h2>
            <p className="mt-1 max-w-2xl text-xs leading-relaxed text-muted-foreground">
              {TOTAL_QUESTIONS} original items across 問題1〜5 (課題理解・ポイント理解・概要理解・即時応答・統合理解),
              timed at {EXAM_MINUTES} minutes and scored out of 60. The scripts are our own text and are played by your
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {CHOKAI_SESSIONS.map((session, index) => (
            <article
              key={session.id}
              data-testid={`n1-chokai-session-${index + 1}`}
            ...
            >
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-muted-foreground">
                  <span className="rounded-full bg-muted px-2.5 py-1">{EXAM_MINUTES} minutes</span>
                  <span className="rounded-full bg-muted px-2.5 py-1">{session.items.length} questions</span>
              ...
              <button
                type="button"
                data-testid={`n1-chokai-start-${index + 1}`}
      ...
      >
        ...
        <div className="mx-auto mt-6 grid max-w-2xl gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-border bg-background p-4 text-center">
            ...
            <p className="mt-1 text-2xl font-black" data-testid="n1-chokai-correct">
              {correctCount} / {TOTAL_QUESTIONS}
            </p>
          </div>
          <div className="rounded-2xl border border-border bg-background p-4 text-center">
            <p className="text-xs text-muted-foreground">Practice conversion</p>
            <p className="mt-1 text-2xl font-black" data-testid="n1-chokai-score">
              {listeningScore} / 60
            </p>
          </div>
          <div className="rounded-2xl border border-border bg-background p-4 text-center">
            <p className="text-xs text-muted-foreground">N1 benchmark</p>
            <p
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                These scripts were written for this app, so they can be shown now that the attempt is submitted —
                read along to check what you missed.
              </p>
              <button
                type="button"
                data-testid="n1-chokai-script-link"
            ...
            {activeSession.items.map((item, index) => (
              <article key={item.id} className="rounded-2xl border border-border bg-background p-4">
                ...
                <p className="mt-2 whitespace-pre-line text-sm leading-6 text-muted-foreground">
                  {item.audio_of
                    ? `（問題5のスクリプトを参照：${scriptOwner(activeSession.items, index).id}）`
                    : item.transcript}
                </p>
  ...
  // ---------------------------------------------------------------------
  return (
    <section data-testid="n1-chokai-exam" className="mt-8">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="shrink-0">
            <p className="text-xs font-bold uppercase text-primary">
              N1 聴解・{activeSession.label}
            </p>
            <p className="mt-1 flex items-center gap-2 font-mono text-xl font-black">
              <Clock className="h-5 w-5" />
              <span data-testid="n1-chokai-timer">{formatTime(secondsLeft)}</span>
            </p>
            ...
            <div className="mt-4 space-y-3">
              {activeSession.items
                ...
                .map((item, localIndex) => {
                  ...
                  const isFollowUp = Boolean(item.audio_of);
                  return (
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0">
                          <p className="font-mono text-[11px] font-bold text-primary">
                            Q{index + 1}
                            {isFollowUp && (
                              <span className="ml-2 font-sans text-[10px] font-bold text-muted-foreground">
                                follows the 問題5 conversation
                              </span>
                            )}
                          </p>
                        ...
                        >
                          {isPlaying ? (
                            ...
                            <>
                              <Play className="h-3.5 w-3.5" /> {isFollowUp ? 'Replay' : 'Play'}
                            </>
                          )}
                        </button>
                      </div>
