...
// Published N1 scoring rules + original auto-scored practice sets written in the
// 問題1〜13 layout. 聴解 is deliberately out of scope for this written-paper
// simulation — it is sat separately below as an original, browser-voiced block.
import { Link } from 'wouter';
import simulationData from '@/lib/realN1SimulationData.json';
import { RealN1Chokai } from './RealN1Chokai';
...
interface TestSet {
  ...
  name: string;
  description: string;
  /** Which sitting the practice bank is styled after, e.g. "2022-12". */
  reference_sitting?: string;
  sections: Section[];
}

const STORAGE_KEY = 'jlpt_n1_simulation_state';

/** Scores the written paper reports back to the exam-day coach. */
export type WrittenPaperResult = {
  setId: string;
  setName: string;
  referenceSitting?: string;
  totalQuestions: number;
  correctCount: number;
  languageScore: number;
  readingScore: number;
  /** Raw answer sheet, kept so the attempt can be reviewed afterwards. */
  answers: Record<string, number>;
};

export interface RealN1SimulationProps {
  /**
   * Exam-day mode: the paper starts immediately (no set picker) and, once
   * submitted, hands its 言語知識 / 読解 scores to the coach instead of ending
   * the sitting locally — the full 合否 is decided after the 聴解 block.
   */
  examDay?: {
    setIndex: number;
    onFinish: (result: WrittenPaperResult) => void;
  };
  /** Review mode: reopen the scored result view for a finished attempt. */
  replay?: {
    setIndex: number;
    answers: Record<string, number>;
...
export function RealN1Simulation({ examDay, replay }: RealN1SimulationProps = {}) {
  const [selectedSetIndex, setSelectedSetIndex] = useState(
    examDay?.setIndex ?? replay?.setIndex ?? 0,
  );
  const [activeSectionId, setActiveSectionId] = useState<string>('all');
  const [viewState, setViewState] = useState<'intro' | 'guidelines' | 'exam' | 'result'>(
    replay ? 'result' : examDay ? 'exam' : 'intro',
  );
  ...
  // Exam-day mode sits the whole written paper straight away: seed the official
  // 110-minute clock on the very first render so the set picker never flashes.
  useEffect(() => {
    if (!examDay) return;
    const minutes = currentSet.sections.reduce(
      (acc, s) => acc + s.official_time_minutes,
      0,
    );
  ...
  const finishExam = () => {
    ...
    setViewState('result');
    if (examDay) {
      // The coach decides the 合否 after 聴解, so the written scores are handed
      // over rather than shown here.
      examDay.onFinish({
        setId: currentSet.id,
        setName: currentSet.name,
        referenceSitting: currentSet.reference_sitting,
        totalQuestions,
        correctCount,
        languageScore: langKnowledgeScore,
        readingScore,
        answers: userAnswers,
      });
    }
  ...
  }, [viewState, currentQIndex, activeQuestions]);
  ...
  // ---------------------------------------------------------------------------
  // JLPT N1 is scored in THREE 60-point blocks:
  //   言語知識（文字・語彙・文法） = 60 marks, benchmark 19
  //   読解                        = 60 marks, benchmark 19
  //   聴解                        = 60 marks, benchmark 19  ← not taken here
  //   total 180 marks, pass mark 90
  // This simulation scores the written paper (言語知識 + 読解 = 120 marks) and
  // reports 聴解 as "not taken", so it can clear the written benchmarks but
  // cannot award a full 合否 verdict on its own.
  // ---------------------------------------------------------------------------
  const langTotal = vocabTotal + gramTotal;
  const langKnowledgeScore = langTotal > 0 ? Math.round(((vocabCorrect + gramCorrect) / langTotal) * 60) : 0;
  const readingScore = readTotal > 0 ? Math.round((readCorrect / readTotal) * 60) : 0;
  const writtenScore = langKnowledgeScore + readingScore;
  const listeningScore = null as number | null;
  const langBenchmarkMet = langKnowledgeScore >= 19;
  const readingBenchmarkMet = readingScore >= 19;
  const writtenBenchmarksMet = langBenchmarkMet && readingBenchmarkMet;
  const isPassed = writtenBenchmarksMet;
          ...
          <div className="flex items-start gap-4 border-b border-border pb-6">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black tracking-tight text-foreground">
                2026 JLPT N1 Exam Day Checklist
              </h1>
              <p className="mt-1 text-sm text-muted-foreground">
                Mandatory rules, strict disqualification triggers, and physical item verification required by JEES & The Japan Foundation.
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 space-y-8">
            ...
            <div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-5">
              ...
              <ul className="mt-3 space-y-2 text-xs md:text-sm text-foreground/90 list-disc list-inside">
                <li>
                  <strong>Envelope Sealing Rule:</strong> Immediately upon entering the test room, all cellphones, smartphones, smart bands, and alarms must be powered completely OFF and sealed inside the paper pouch provided by proctors.
                </li>
                <li>
                ...
                <li>
                  <strong>聴解 happens separately:</strong> the listening paper is sat after the written paper and the room is locked once the audio starts. This simulation covers the written paper only, so run 聴解 with the original listening block further down this page when you practise it.
                </li>
              </ul>
            </div>
            ...
            <div>
              <h3 className="text-lg font-bold flex items-center gap-2 text-foreground">
                ...
                N1 Timings & Sectional Pass Matrix (per JLPT published guidelines)
              </h3>
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-4 rounded-2xl border border-border bg-background text-center">
                  <p className="text-xs text-muted-foreground font-mono uppercase">1. 言語知識・読解</p>
                  <p className="text-2xl font-black text-foreground mt-1">110 Minutes</p>
                  <p className="text-[11px] text-muted-foreground mt-1">The whole written paper, one sitting</p>
                </div>
                <div className="p-4 rounded-2xl border border-border bg-background text-center">
                  <p className="text-xs text-muted-foreground font-mono uppercase">2. 聴解 (Listening)</p>
                  <p className="text-2xl font-black text-foreground mt-1">60 Minutes</p>
                  <p className="text-[11px] text-muted-foreground mt-1">55 Minutes from 2022-12 on — 聴解 is sat in the archive below</p>
                </div>
                <div className="p-4 rounded-2xl border border-border bg-background text-center">
                  <p className="text-xs text-muted-foreground font-mono uppercase">Practice split</p>
                  <p className="text-2xl font-black text-foreground mt-1">35 + 35 + 40</p>
                  <p className="text-[11px] text-muted-foreground mt-1">文字・語彙 / 文法 / 読解 (extra practice only)</p>
                </div>
              </div>
            </div>
          </div>
  ...
  if (viewState === 'exam') {
    ...
    return (
      <div className="mx-auto max-w-[1020px] px-4 py-6 pb-28 md:px-8 md:py-8 md:pb-16 animate-fadeIn" data-testid="n1-sim-exam">
        {/* Status & Timer Bar */}
...
{q.section.toUpperCase()}
  ...
  if (viewState === 'result') {
    ...
    return (
      <div className="mx-auto max-w-[960px] px-4 py-8 pb-28 md:px-8 md:py-12 md:pb-16 animate-fadeIn" data-testid="n1-sim-result">
        <div className={`rounded-3xl border p-6 md:p-8 text-center shadow-lg transition ${
          ...
          <h2 className="text-2xl md:text-3xl font-black text-foreground">
            {isPassed ? 'N1 WRITTEN PAPER CLEARED — Benchmarks Met!' : 'N1 WRITTEN PAPER BELOW BENCHMARK'}
          </h2>
          <p className="mt-2 text-sm text-muted-foreground max-w-xl mx-auto">
            {isPassed
              ? `You met both written benchmarks: 言語知識 ${langKnowledgeScore}/60 and 読解 ${readingScore}/60 (基準点 ≥ 19 each = 38/120 on the written paper).`
              : `The published 基準点 is ≥ 19/60 in every block: 言語知識 ${langKnowledgeScore}/60, 読解 ${readingScore}/60. 聴解 is not taken here, so a full 合否 verdict needs the real listening paper too.`}
          </p>
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-2xl mx-auto">
            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              <p className="text-[11px] font-mono text-muted-foreground uppercase">言語知識（文字・語彙・文法）</p>
              <p className="text-2xl font-black text-foreground mt-1" data-testid="n1-lang-score">{langKnowledgeScore} / 60</p>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${langBenchmarkMet ? 'bg-emerald-500/20 text-emerald-600' : 'bg-destructive/20 text-destructive'}`}>
                基準点 ≥ 19
              </span>
            </div>

            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              <p className="text-[11px] font-mono text-muted-foreground uppercase">読解 (Reading)</p>
              <p className="text-2xl font-black text-foreground mt-1" data-testid="n1-reading-score">{readingScore} / 60</p>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${readingBenchmarkMet ? 'bg-emerald-500/20 text-emerald-600' : 'bg-destructive/20 text-destructive'}`}>
                基準点 ≥ 19
              </span>
            </div>
            ...
            <div className="p-3.5 rounded-2xl border border-border bg-card text-center">
              ...
              <p className="text-2xl font-black text-foreground mt-1" data-testid="n1-listening-score">{listeningScore === null ? 'Not taken' : `${listeningScore}/60`}</p>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                未受験 — excluded from scoring
              </span>
            </div>
          </div>

          <div className="mt-5 rounded-2xl border border-border bg-card p-4 text-xs text-muted-foreground">
            <strong className="text-foreground">Written paper: {writtenScore} / 120</strong> (言語知識 + 読解, 基準点合計 ≥ 38).
            A full N1 合否 needs the 聴解 block as well: ≥ 19/60 there and ≥ 90/180 overall.
          </div>
        ...
        <div className="mt-10">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Japanese explanations for every 問題1〜13 item, in 問題 order.</p>
            </div>

            <div className="flex items-center gap-1.5 rounded-xl border border-border p-1 bg-card text-xs">
              <button
                data-testid="n1-filter-all"
              ...
              <button
                data-testid="n1-filter-incorrect"
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-bold text-muted-foreground flex items-center gap-1">
              <Calendar className="h-3.5 w-3.5" /> N1 Timings & Guidelines
            </span>
          </div>
          <h1 className="mt-2 text-2xl md:text-3xl font-black tracking-tight text-foreground">
JLPT N1 Written Examination Simulation
          </h1>
          <p className="mt-1 text-sm text-muted-foreground max-w-2xl">
            Sit the N1 written paper as one 110-minute block, or drill a single part with a practice split (文字・語彙 35分 / 文法 35分 / 読解 40分). Every set is an original 問題1〜13 bank scored against the published N1 得点区分 — 言語知識 60点 and 読解 60点, 基準点 19点 each.
          </p>
        </div>
      ...
      {/* Full exam day: written paper -> break -> 聴解 -> one 合否 verdict */}
      <Link
        href="/jlpt-simulation/n1/exam-day"
        data-testid="n1-exam-day-banner"
        className="mt-8 flex flex-col gap-3 rounded-3xl border border-primary/40 bg-primary/5 p-5 transition hover:border-primary hover:bg-primary/10 md:flex-row md:items-center md:justify-between"
      >
        <div className="flex items-start gap-3">
          <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-primary/15 text-primary">
            <ShieldAlert className="h-5 w-5" />
          </span>
          <div>
            <p className="text-sm font-black text-foreground">
              Full exam day — 言語知識・読解 110分 → 聴解 55分 → one 合否 verdict
            </p>
            <p className="mt-1 max-w-2xl text-xs text-muted-foreground">
              Sit both halves back to back. Scores stay hidden until the 聴解
              block is done, then you get the published three-block result:
              total ≥ 90/180 with 基準点 ≥ 19/60 in 言語知識・読解・聴解.
            </p>
          </div>
        </div>
        <span className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-primary-foreground">
          Start exam day <ArrowRight className="h-3.5 w-3.5" />
        </span>
      </Link>
              ...
              >
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-lg border border-primary/40 bg-primary/5 px-2.5 py-1.5 text-[11px] font-bold text-primary">
                      <FileText className="h-3.5 w-3.5" />
                      Original practice bank
                    </span>
                    <span className="text-[11px] font-mono text-muted-foreground">
                      styled after {set.reference_sitting} · 問題1〜13 · 聴解 not included
                    </span>
                  </div>
                </div>
          ...
          >
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                The whole 言語知識・読解 paper under real conditions
              </p>
            </div>
      ...
      {/* Original 聴解 block (browser voice) */}
      <RealN1Chokai />
    </div>
  );
}