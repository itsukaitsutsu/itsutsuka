       ...
       <section className="animate-pop relative overflow-hidden rounded-[1.75rem] border border-border bg-card p-6 shadow-[0_18px_50px_hsl(var(--primary)/.05)] md:p-8" data-testid={testId('quiz-setup')}>
        ...
        <div className="mt-8 space-y-7">
           ...
           {title === 'Casual' && <section className="rounded-2xl border border-[hsl(var(--secondary)/.25)] bg-[hsl(var(--secondary)/.04)] p-4" data-testid="aim-shooter-settings">
             <div><h3 className="text-sm font-bold">Aim shooter difficulty</h3><p className="mt-1 text-xs text-muted-foreground">Set target shuffle timing, travel speed and animation FPS before the round.</p></div>
             ...
             <button onClick={() => setBossFight((enabled) => !enabled)} aria-pressed={bossFight} className={cx('mt-3 flex w-full items-center justify-between gap-3 rounded-xl border p-3 text-left transition-colors', bossFight ? 'border-[hsl(var(--accent))] bg-[hsl(var(--accent)/.1)]' : 'border-border bg-card/70')} data-testid="toggle-aim-boss-fight"><span><span className="block text-xs font-bold">Boss fight</span><span className="mt-1 block text-[10px] text-muted-foreground">The boss fires at you; dodge with WASD and jumps. Correct answers damage it.</span></span><span className={cx('rounded-lg px-2.5 py-1.5 text-[10px] font-black', bossFight ? 'bg-[hsl(var(--accent))] text-[hsl(var(--foreground))]' : 'bg-muted text-muted-foreground')}>{bossFight ? 'ON' : 'OFF'}</span></button>
           </section>}
        </div>
