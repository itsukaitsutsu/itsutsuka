...
import { AimShooterMode } from '@/components/AimShooterMode';
...
function QuizSetupPanel({ title }: { title: 'Casual' | 'Ranked' }) {
  ...
  const [targetSwapSeconds, setTargetSwapSeconds] = useState(4);
  const [targetMovementSpeed, setTargetMovementSpeed] = useState(5);
  const [shooterFrameCap, setShooterFrameCap] = useState<30 | 60 | 120>(60);
  const [bossFight, setBossFight] = useState(true);
  const [bossDifficulty, setBossDifficulty] = useState(5);
    ...
    <div className="grid gap-8 lg:grid-cols-1 lg:items-start">
       <section className="animate-pop relative overflow-hidden rounded-[1.75rem] border border-border bg-card p-6 shadow-[0_18px_50px_hsl(var(--primary)/.05)] md:p-8" data-testid={testId('quiz-setup')}>
        ...
        <div className="mt-8 space-y-7">
           ...
           {title === 'Casual' && <section className="rounded-2xl border border-[hsl(var(--secondary)/.25)] bg-[hsl(var(--secondary)/.04)] p-4" data-testid="aim-shooter-settings">
             <div><h3 className="text-sm font-bold">Aim shooter difficulty</h3><p className="mt-1 text-xs text-muted-foreground">Set target timing, movement speed, FPS cap and boss difficulty before the round.</p></div>
             <div className="mt-4 rounded-xl border border-border/70 bg-card/70 p-3"><div className="flex items-center justify-between gap-3"><div><label htmlFor="aim-target-swap-seconds" className="text-xs font-bold">Swap positions every</label><p className="mt-1 text-[10px] text-muted-foreground">How often the answer options relocate</p></div><span className="rounded-lg border border-[hsl(var(--secondary)/.25)] bg-card px-3 py-2 text-sm font-black text-[hsl(var(--secondary))]">{targetSwapSeconds}s</span></div><input id="aim-target-swap-seconds" type="range" min="1" max="10" step="1" value={targetSwapSeconds} onChange={(event) => setTargetSwapSeconds(Number(event.target.value))} className="mt-3 w-full accent-[hsl(var(--secondary))]" data-testid="input-aim-target-swap"/><div className="flex justify-between text-[10px] font-semibold text-muted-foreground"><span>1s · Harder</span><span>10s · Easier</span></div></div>
             <div className="mt-3 rounded-xl border border-border/70 bg-card/70 p-3"><div className="flex items-center justify-between gap-3"><div><label htmlFor="aim-target-movement-speed" className="text-xs font-bold">Target movement speed</label><p className="mt-1 text-[10px] text-muted-foreground">How quickly targets glide to their next position</p></div><span className="rounded-lg border border-[hsl(var(--secondary)/.25)] bg-card px-3 py-2 text-sm font-black text-[hsl(var(--secondary))]">{targetMovementSpeed}/10</span></div><input id="aim-target-movement-speed" type="range" min="1" max="10" step="1" value={targetMovementSpeed} onChange={(event) => setTargetMovementSpeed(Number(event.target.value))} className="mt-3 w-full accent-[hsl(var(--secondary))]" data-testid="input-aim-target-movement-speed"/><div className="flex justify-between text-[10px] font-semibold text-muted-foreground"><span>1 · Slow</span><span>10 · Fast</span></div></div>
             <div className="mt-3 rounded-xl border border-border/70 bg-card/70 p-3"><p className="text-xs font-bold">Animation FPS limit</p><p className="mt-1 text-[10px] text-muted-foreground">Maximum update rate for moving targets</p><div className="mt-3 grid grid-cols-3 gap-2">{([30, 60, 120] as const).map((fps) => <button key={fps} onClick={() => setShooterFrameCap(fps)} aria-pressed={shooterFrameCap === fps} className={cx('rounded-lg border py-2.5 text-xs font-black transition-colors', shooterFrameCap === fps ? 'border-[hsl(var(--secondary))] bg-[hsl(var(--secondary)/.12)] text-[hsl(var(--secondary))]' : 'border-border text-muted-foreground hover:bg-muted')} data-testid={`aim-frame-cap-${fps}`}>{fps} FPS</button>)}</div><p className="mt-2 text-[10px] leading-4 text-muted-foreground">The game may run below this cap if your browser or display cannot sustain it.</p></div>
             <button onClick={() => setBossFight((enabled) => !enabled)} aria-pressed={bossFight} className={cx('mt-3 flex w-full items-center justify-between gap-3 rounded-xl border p-3 text-left transition-colors', bossFight ? 'border-[hsl(var(--accent))] bg-[hsl(var(--accent)/.1)]' : 'border-border bg-card/70')} data-testid="toggle-aim-boss-fight"><span><span className="block text-xs font-bold">Boss fight</span><span className="mt-1 block text-[10px] text-muted-foreground">Boss starts with one HP per question. Correct answers damage it; dodge its shots with WASD and jumps.</span></span><span className={cx('rounded-lg px-2.5 py-1.5 text-[10px] font-black', bossFight ? 'bg-[hsl(var(--accent))] text-[hsl(var(--foreground))]' : 'bg-muted text-muted-foreground')}>{bossFight ? 'ON' : 'OFF'}</span></button>
             {bossFight && <div className="mt-3 rounded-xl border border-border/70 bg-card/70 p-3"><div className="flex items-center justify-between gap-3"><div><label htmlFor="aim-boss-difficulty" className="text-xs font-bold">Boss aim &amp; fire difficulty</label><p className="mt-1 text-[10px] text-muted-foreground">Harder means faster volleys, tighter aim, and bursts up to 8 shots</p></div><span className="rounded-lg border border-[hsl(var(--secondary)/.25)] bg-card px-3 py-2 text-xs font-black text-[hsl(var(--secondary))">{bossDifficulty}/10</span></div><input id="aim-boss-difficulty" type="range" min="1" max="10" step="1" value={bossDifficulty} onChange={(event) => setBossDifficulty(Number(event.target.value))} className="mt-3 w-full accent-rose-500" data-testid="input-aim-boss-difficulty"/><div className="flex justify-between text-[10px] font-semibold text-muted-foreground"><span>1 · Easy</span><span>10 · Hard</span></div></div>}
           </section>}
        </div>
